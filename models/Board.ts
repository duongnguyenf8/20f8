...
export class Board {
  ...
  spawnValue(): void {
    ...
    const notEmptyFields = this.getNotEmptyFields()
    const rate = 0.9
    switch (+notEmptyFields.length) {
      case 8: {
        return emptyFields[idx].spawn(Math.random() < rate ? 4 : 8)
      }
      case 12: {
        return emptyFields[idx].spawn(Math.random() < rate ? 8 : 16)
      }
      default: {
        return emptyFields[idx].spawn(Math.random() < rate ? 2 : 4)
      }
    }
  }
  ...
  private getNotEmptyFields(): Field[] {
    return this._fields.filter((filed) => !filed.isEmpty())
  }
